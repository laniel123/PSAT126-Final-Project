...
**Histogram Comments:**
Carats: The carat distribution is clearly right-skewed. Most observations fall in the 0–1 range, with a peak around the 1.0 and 1.5 interval with a smaller increase near 1.5. We have an outlier.The distribution matches with real world expectation taht the larger the carat the rarer the diamond is. 


Depth: Depth values look roughly symmetric and close to a normal shape, centered around the typical range, with one visible outlier slightly above 70. Besides that, the distribution is pretty regular without major irregularities.

Table: The table distribution is slightly right-skewed. Most values are clustered between 54–60, with around 55 being the most common. This suggests that the majority of diamonds fall within a standard table proportion range.

Prices: Price is heavily right-skewed. Most diamonds in the sample cost under $5,000, but there is a long upper tail with many diamonds priced above $10,000. This pattern is expected because a small number of diamonds are very expensive. There aren’t any extreme outliers beyond this tail, so the distribution looks reasonable.
...
Run multiple linear regression model using all these variables and observe the summary
...
Comment on anything of interest that occurred in this part. Were the data approximately
...
Start with one predictor and one response from the variables you chose in Part I. For
...
## 2. 
Run the model and examine the summary statistics, interpreting everything (hypothesis
...
## 3.
Test the assumptions and apply any necessary transformations to the response variable y
or the predictor.
...
## 4. 
Call the summary function on the transformed variables, observe the summary, and note any changes.
...
Add other variables to the model and assess if the model improves. For step 5, run the code
...
Comment anything of interest while doing this.
...
Based on the best model obtained from Part II (you would have more than one variable now), run it and call the summary function to analyze how it works and what you observe.

## 2. 

Detect multicollinearity among the variables using the variance inflation factor (VIF)

## 3.

Give CIs for a mean predicted value and the PIs of a future predicted value for at least one
