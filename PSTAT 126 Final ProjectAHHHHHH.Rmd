...
summary(AIC_model)
```
The variable cut was eliminated from the model using the stepwise AIC approach. The improved model fit under the AIC criterion is demonstrated by the modified model's lower AIC value (AIC = –1108.76). The model still includes the predictors carat, depth, table, color, and clarity. Carat continues to be the best predictor, and the majority of clarity and color levels have statistically significant influence on log(price).

With fewer predictors, the final model retains a high adjusted R^2 value (0.8959), indicating that the reduced model explains almost as much variability as the full model. The chosen model strikes a balance between model fit and parsimony because AIC penalizes model complexity.
